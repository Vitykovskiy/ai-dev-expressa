...
`2026-05-02: выполнен Vuetify-first audit и поведенчески нейтральный frontend alignment. Проверены official tooling routes Vue 3 guide, Vuetify components catalog, Vuetify buttons/button-groups/radio-buttons и существующий frontend/src/ui catalog. Неиспользуемые legacy shell-дубли frontend/src/components/TopBar.vue, SideNav.vue и TabBar.vue удалены, потому что canonical shell primitives уже находятся в frontend/src/ui. UsersFilterTabs переведен с native button markup на Vuetify v-btn-toggle/v-btn. AssignRoleDialog переведен с native radio input на Vuetify v-radio-group/v-radio внутри существующего ui-dialog-shell. SlotSettingsForm переведен с локального v-text-field styling на ui-text-field; ui-text-field расширен только пропсами inputmode/error поверх Vuetify v-text-field. Сохраненные custom ui-* primitives оставлены как typed backoffice-wide adapters над Vuetify для shell, dialog, section, field и action contracts, чтобы не протекали лишние Vuetify details в feature-код. BUG-004 table scope не затрагивался. Routes, guards, actor/capabilities, API shapes и error mapping не менялись. Проверки: npm run lint:frontend — passed; npm run stylelint:frontend — passed; npm run format:check:frontend — passed; npm run typecheck:frontend — passed; npm run test:frontend — 15 files/71 tests passed; npm run build:frontend — passed; manual route behavior check — router/navigation/auth files не изменялись, routes /, /availability, /menu, /users, /settings, /forbidden, /entry-denied сохранены.`
