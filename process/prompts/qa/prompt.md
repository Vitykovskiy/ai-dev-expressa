...
- Read `docs/architecture/deployment-map.md` when the task changes or validates an environment, pipeline, deployment path, smoke-check, rollback, restore path, or documented e2e run path.
...
- Do not close feature-level e2e QA using only an in-process backend run when the assigned task requires browser e2e evidence through frontend and backend.
- Do not use the historical/deprecated `test` VPS e2e route as QA acceptance evidence when the assigned task requires local containerized e2e.
...
- For e2e QA tasks, adapt the e2e suite to the documented verification path; for `QA-005`, this path is local containerized e2e.
- For e2e QA tasks, run e2e through the documented route, record the run result, and file reproducible defects for every product issue or launch issue you find.
...
- Mandatory checks for e2e QA tasks are e2e-tests for the assigned feature and explicit confirmation that they pass on the assembled result; when a local containerized route is documented or required by the task, that route is the mandatory acceptance evidence.
...
- Use contour label `frontend` for UI/client defects, `backend` for API/contract/server defects, and `devops` for environment, deployment, env/config, smoke-check, pipeline path, local container runner, Docker runtime or test runner launch defects.
