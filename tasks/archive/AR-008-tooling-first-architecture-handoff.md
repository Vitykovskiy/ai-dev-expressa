...
## Handoff для дочерних задач

- `FE-008` использует `Vue 3`, `Vuetify` и `Vitest`; официальный маршрут проверки находится в `docs/architecture/stack.md` и `docs/architecture/frontend-architecture.md`; canonical UI primitive layer — `frontend/src/ui`, рабочий каталог — `frontend/src/ui/README.md`.
- `BE-007` использует `NestJS` и `Vitest`; официальный маршрут проверки находится в `docs/architecture/stack.md` и `docs/architecture/backend-architecture.md`; public API contracts, DTO shape, auth semantics и error mapping сохраняются.
- `DO-010` использует `Docker Compose`, GitHub Actions, npm scripts, Husky и lint-staged; официальный маршрут проверки находится в `docs/architecture/stack.md`, `docs/architecture/devops-standards.md` и `docs/architecture/application-map/delivery-and-runtime.md`; runtime semantics и изоляция стендов сохраняются.
- `QA-010` и `QA-011` используют manual QA route, `Playwright` и `Vitest` evidence; официальный маршрут проверки находится в `docs/architecture/qa-standards.md` и `docs/architecture/application-map/qa-*.md`; воспроизводимые дефекты оформляются отдельными `BUG-*` с контуром причины.
...
`2026-05-02: архитектурные документы обновлены под tooling-first handoff. Исправлен canonical UI primitive path с frontend/src/components/ui на frontend/src/ui, добавлены official documentation routes для Vue 3, Vuetify, NestJS, Docker Compose, GitHub Actions, npm scripts, Husky, lint-staged, Playwright и Vitest. Контурные границы FE/BE/DO/QA зафиксированы без изменения process-layer, production-кода, публичных контрактов или runtime semantics. Проверки: rg "frontend/src/components/ui" docs/architecture — совпадений нет; rg "frontend/src/ui" docs/architecture frontend/src/ui/README.md — путь найден; ручная architecture consistency check выполнена.`
