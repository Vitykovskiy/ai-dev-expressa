...
- Источники: `feature spec / Main workflow`, `docs/system/use-cases/administrator-manage-users-and-roles.md`, `.references/Expressa_admin/src/app/screens/UsersScreen.tsx`
...
- Ожидаемый результат:
  ...
  2. `Система должна показать список пользователей, доступный для выбора цели назначения роли.`
...
- E2E mapping:
  ...
  - Required assertions: `проверка наличия вкладки users для administrator, проверка отображения списка пользователей, проверка доступности кнопки Добавить пользователя`
...
- Ожидаемый результат:
  ...
  2. `Система должна сохранить новое ролевое назначение без частичного результата.`
  3. `Система должна пересчитать доступ пользователя так, чтобы после успешного назначения были доступны только вкладки Заказы и Доступность в рамках подтвержденной доменной модели.`
...
- E2E mapping:
  ...
  - Required assertions: `проверка успешной отправки формы с role=barista, проверка success notification, проверка обновленного набора доступных вкладок для целевого пользователя`
...
- Ожидаемый результат:
  1. `Система должна отклонить изменение роли с ошибкой role-not-assignable.`
...
- E2E mapping:
  ...
  - Required assertions: `проверка business error role-not-assignable, проверка отсутствия success notification, проверка неизменности набора ролей пользователя`
...
- Предусловия: пользователь аутентифицирован без роли `administrator` и без capability `users`.
...
- Ожидаемый результат:
  ...
  2. `Система должна отклонить прямую попытку входа в защищенный контур или вызов операции назначения роли.`
  3. `Система должна вернуть наблюдаемую ошибку administrator-role-required без изменения ролей целевого пользователя.`
- Проверяемые инварианты:
...
- E2E mapping:
  ...
  - Required assertions: `проверка отсутствия вкладки users, проверка protected или forbidden state при прямом доступе, проверка business error administrator-role-required`
...
- Ожидаемый результат:
  ...
  3. `Система должна передать blocker в архитектурный handoff явно, а не скрывать его за UI-наличием выбора роли.`
- Проверяемые инварианты:
