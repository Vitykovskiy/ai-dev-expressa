...
- Read the task's `Маршрут чтения`, the assigned role prompt, the contour map, the assigned `Зона ответственности`, and targeted documents for the assigned slice.
...
- Start with the source task card, its `Маршрут чтения` field, the profile prompt for the role, the contour map, and targeted documents.
...
- Apply the tooling-first rule from `process/workflow.md`; if project documentation names official tool references for the assigned contour, include the relevant ones in the context package when the subtask touches framework extension points, runtime wiring, persistence integration, testing infrastructure, delivery tooling, UI-kit composition patterns, or other non-trivial tool behavior.
...
- The allowed edit area must be derived from the source task `Зона ответственности`, role prompt, contour map, and assigned slice.
