...
- Зависимости: `нет`
- Согласованный состав дочерних задач: `AR-001` — архитектурная рамка Telegram-входа и guard-правил; `BE-001` — bootstrap главного administrator и серверный контроль доступа; `FE-001` — инициализация `apps/backoffice-web`; `FE-002` — HTTP-сервисы bootstrap доступа и маршрутизация `apps/backoffice-web`; `FE-003` — ролевые guard-правила и экран отказа в доступе `apps/backoffice-web`; `FE-009` — исправление browser-only bootstrap в test-окружении VPS при относительном API base URL; `DO-001` — окружение, конвейер и дымовая проверка FEATURE-001; `QA-001` — e2e-покрытие Telegram-входа administrator.
- Рекомендованный порядок выполнения: `AR-001`, затем параллельно `BE-001` и `FE-001`, после чего `FE-002`, затем `FE-003`, после чего `DO-001` и `QA-001`.
