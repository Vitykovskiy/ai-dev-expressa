...
- Декомпозиция: `AR-006`, `FE-006`, `BE-005`, `QA-006`, `QA-007`, `DO-009`.
...
- Критерии готовности: `Фича закрыта, когда настройки слотов сохраняются и применяются к выдаче слотов, а дочерние AR/FE/BE/QA/DO-* задачи завершены и проверены.`
...
- `AR-006`, `BE-005`, `FE-006`, `QA-006`, `QA-007` и связанная `DO-009` завершены.
- Administrator может читать, изменять и сохранять рабочие часы и вместимость слотов во вкладке `Настройки`.
- Последний успешно сохраненный snapshot влияет на дальнейшую генерацию доступных customer-слотов текущего дня.
- Финальный acceptance route подтвержден: `npm run test:e2e -- slot-settings` завершился с результатом `5 passed` против published `test-e2e` stand.
