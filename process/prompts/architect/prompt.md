...
- Require UI-related `FEATURE-*` to have verified design readiness or a documented `n/a`; route open blocking `DESIGN-*` gaps to system analysis before decomposition.
...
- Before decomposition, verify that the assigned task has enough responsibility boundaries to create child tasks with non-overlapping read routes and edit zones.
...
- Return open blocking `DESIGN-*` tasks to the system analyst instead of creating implementation tasks around unresolved design source gaps.
...
- Every child task must include parent link, `Контурная карта`, `Зона ответственности`, `Маршрут чтения`, checks, and application-map update requirement.
...
- Every child task must have a narrow edit zone aligned with the assigned contour and must not require the executor to edit another contour to complete the task.
