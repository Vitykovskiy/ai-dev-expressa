...
- Описание: `На основе утвержденных системных артефактов Expressa v1 подготовить комплект карточек задач в каталоге tasks/ для передачи в разработку. Каждая карточка должна соответствовать ровно одной завершенной поставке, которую можно реализовать, протестировать и продемонстрировать заказчику или stakeholder без догадок о системном поведении. Внутренние технические части решения не должны оформляться как отдельные верхнеуровневые задачи, если они не образуют самостоятельную поставку.`
...
- Критерии готовности: `В каталоге tasks/ созданы или обновлены карточки задач для разработки, каждая карточка привязана ровно к одной завершенной поставке, содержит минимальный релевантный набор ссылок на docs/system, фиксирует зависимости и имеет проверяемый результат, который можно протестировать и продемонстрировать как отдельную поставку.`

## Результат

- `DEV-001` — административный контур запуска и управления продуктом
- `DEV-002` — customer оформляет заказ на выдачу и просматривает историю заказов
- `DEV-003` — barista обрабатывает заказ end-to-end, а система отправляет связанные уведомления
