...
- Modify and, when necessary, create only the allowed zones for this role: canonical system artifacts in `docs/system/`, the system documentation map `docs/system/README.md`, and `SPRINT-*`/`FEATURE-*` task cards in `tasks/`.
- Use the assigned task, its `Маршрут чтения` or legacy `Минимальный read set`, and explicitly assigned role instructions as the task-specific source route.
...
- Read the task.
- Read artifacts from the assigned `Маршрут чтения` or legacy `Минимальный read set`.
...
- Legacy flat feature specs and sibling `.test-scenarios.md` are valid only for existing features until the next system-analysis update.
- Minimum completeness:
