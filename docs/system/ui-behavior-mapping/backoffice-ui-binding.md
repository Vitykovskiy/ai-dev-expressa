...
| `users`                                 | `open AssignRoleDialog` + `confirm role=administrator`                                                                        | `Assign user role`                             | Вход UI для выбора роли `administrator` присутствует в React-референсе; финальное guard-правило назначения `administrator` остается системным blocker.          |
