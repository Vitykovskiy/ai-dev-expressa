...
- In-memory adapters допустимы только как временные development/test doubles внутри локальных тестов и не являются допустимой production-реализацией feature-level handoff.
...
- Нельзя считать in-memory хранение допустимым конечным решением для production-ready feature; если feature меняет доменные данные, её handoff должен учитывать реализацию через `PostgreSQL`.
