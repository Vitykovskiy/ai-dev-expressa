...
- Ожидаемый результат для ревью: `Собранная delivery unit DU-01 с child-задачами архитектора, frontend, backend, devops и QA дает рабочий административный контур, который можно отдельно развернуть, проверить и показать stakeholder.`
- Проверки: `Собранный результат проходит обязательные проверки child-задач FE/BE/DO и финальный приемочный сценарий administrator.`
- Обновление карты приложения: `Обязательно в child-задачах, если появляются новые модули, entrypoints, env/config или deployment path.`
