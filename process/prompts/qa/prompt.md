...
- For manual QA tasks, record the tester report in the assigned `QA-*` task against stable scenario IDs from the feature test scenarios document.
...
- Manual QA results in the `QA-*` task must reference scenario IDs from the feature test scenarios document.
- E2E QA results in the `QA-*` task must reference scenario IDs from the feature test scenarios document and list the mapped test files, test titles and required assertions.
