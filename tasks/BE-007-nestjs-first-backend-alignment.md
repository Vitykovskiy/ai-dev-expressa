...
2026-05-02: backend-контуры identity-access, menu-catalog, slot-settings и config сверены с назначенным NestJS-first маршрутом. Проверены штатные возможности NestJS modules/controllers/providers/guards/testing и Vitest guide: production-код уже использует NestJS module/controller/provider/guard boundaries, поэтому public API, DTO, auth semantics, status codes и error mapping не менялись. Backend tests для injectable services/validators переведены на `@nestjs/testing` `Test.createTestingModule`: `backoffice-auth.spec.ts`, `bootstrap-administrator.spec.ts`, `menu-catalog-domain.spec.ts`, `slot-settings-domain.spec.ts`. Кастомные domain validators, in-memory repositories, access/config validators и pure unit tests сохранены, потому что штатные NestJS utilities покрывают DI/testing boundary, но не заменяют проектные domain invariants, error contracts и текущий in-memory storage adapter, разрешенный architecture map. Проверки пройдены: `npm run lint:backend`, `npm run format:check:backend`, `npm run typecheck:backend`, `npm run test:backend`, `npm run build:backend`.

R03 `2026-05-02`: для unblock manual QA добавлен test-mode bootstrap fixture `testTelegramId=777008` с ролью `barista`, доступный только при `NODE_ENV=test DISABLE_TG_AUTH=true`. Причина кастомной fixture: в текущих contracts нет flow создания второго пользователя, а существующий public session contract уже поддерживает вход только для существующего `testTelegramId`; fixture подготавливает такого пользователя без нового endpoint, DTO, status/error mapping или production bypass. Проверки resolver пройдены: `npm run lint:backend`, `npm run format:check:backend`, `npm run typecheck:backend`, `npm run test:backend`.
