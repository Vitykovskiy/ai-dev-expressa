...
| `backend/src/identity-access/users/postgresql-user.repository.ts`          | PostgreSQL-адаптер хранения пользователей, ролей и `blocked state`; канонический runtime path `FEATURE-004` для users boundary.                                      |
...
- Users boundary должен читать и сохранять пользователей, роли и `blocked state` через `PostgreSQL`; локальные test doubles не определяют runtime contract этой фичи.
