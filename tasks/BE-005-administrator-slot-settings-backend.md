...
- Проверки: `Выполнено: модульные тесты доменных правил slot settings и slot generation; integration tests для endpoints чтения/сохранения настроек и customer slot generation; negative tests для invalid-working-hours, invalid-slot-capacity и доступа без capability settings; backend lint, format:check, build, typecheck и test.`
...
- Реализован backend-контур `slot-settings` с `GET/PUT /backoffice/settings/slot-settings` под capability `settings`.
- Реализован `GET /customer/slots`, использующий последнее успешно сохраненное сочетание рабочих часов и вместимости для генерации 10-минутных слотов текущего дня.
- Дефолтные значения `09:00–20:00` и `5` возвращаются до первого сохранения; верхняя граница `slotCapacity` не канонизирована сверх подтвержденного правила положительного целого значения.
- Добавлены unit и integration tests, покрывающие defaults, partial update, invalid-working-hours, invalid-slot-capacity, guard `settings` и влияние сохранения на дальнейшую генерацию слотов.
