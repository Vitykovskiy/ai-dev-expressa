...
Одна карта привязки: customer-facing Telegram WebApp для выбора меню, корзины, слотов и истории заказов.
...
- Этот артефакт связывает экранное поведение с системными use cases, contracts, state-models и бизнес-правилами.
...
## Навигация к use cases

| Переход UI | Системный смысл |
|---|---|
| `welcome -> menu_categories` | Вход в сценарий создания заказа. |
| `menu_categories -> menu_category_products -> product -> cart -> slot_picker` | Сквозной системный use case создания заказа. |
