...
| E2E                      | Browser suite через опубликованный backoffice: administrator создает категорию, товар-напиток с ценами `S/M/L`, группу дополнительных опций, опции и назначает группу на категорию; финальное evidence для `QA-005` собирается на isolated route deployed `test` VPS. |
...
- Финальный e2e acceptance для `QA-005` подтверждается полным browser suite на isolated route задеплоенного `test` VPS после успешного deploy и smoke-check.
