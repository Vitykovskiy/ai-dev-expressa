...
| `users`             | Назначение ролей и управление статусом доступа      | `administrator-manage-users-and-roles`, `administrator-block-user`                                 | `Assign user role`, `Block user`                                                                         | `AR-004`–`AR-010`, `BO-006`                                          |
...
| `users`                                 | `assign_administrator`                                                                                                        | `Assign user role`                         | Допустимо только для `BootstrapAdministrator`; при отсутствии этого guard система возвращает `administrator-role-assignment-forbidden`.                         |
| `users`                                 | `block_user`                                                                                                                  | `Block user`                               | Переводит пользователя в blocked-состояние.                                                                                                                     |
| `users`                                 | `unblock_user`                                                                                                                | Смежный flow user-access-status management | Подтвержден как соседняя операция на том же экране `Пользователи`, но остается в отдельной feature boundary и не меняет scope `FEATURE-004`.                    |
...
| `users.action_success.toast`                           | `Assign user role`, `Block user`                   | Успешные административные операции на экране `Пользователи` должны быть подтверждаемы для оператора.   |
...
- UI-контракт backoffice визуально объединяет `assign role`, `block` и `unblock` на экране `Пользователи`; системная аналитика подтверждает эту единую business-surface, но сохраняет раздельные feature boundaries для `FEATURE-004` и соседнего потока управления blocked state.
- UI-контракт backoffice должен показывать действие назначения роли `administrator` в рамках того же entrypoint `Добавить пользователя`, при этом системный guard ограничивает это действие `BootstrapAdministrator`.
