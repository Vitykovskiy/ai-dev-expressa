...
- Поле `Роль` в карточке `FEATURE-*` должно иметь значение `Архитектор`.
...
- Статус `Готова к работе` у `FEATURE-*` означает готовность к архитектурной декомпозиции.
- Первое исполнительное действие по `FEATURE-*` со статусом `Готова к работе` выполняется в роли архитектора.
