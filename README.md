...
- Дочерние карточки `AR/FE/BE/DO/QA` ссылаются на родительскую `DEV-*` через поле `Родительская задача` и описывают один проверяемый результат в одном контуре.
- Системный аналитик готовит родительскую delivery unit, архитектор фиксирует стек и кодстайл, обновляет архитектурные артефакты и нарезает дочерние задачи для реализации.
