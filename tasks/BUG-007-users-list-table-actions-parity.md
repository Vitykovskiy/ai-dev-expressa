...
`2026-05-03: исправлена frontend-parity вкладки Пользователи: фильтры переведены к pill-виду по прототипу, список собран через UiDataTable, в строках пользователей отображается кнопка меню действий, сценарий Назначить роль сохранен. Проверки: npm run test:frontend прошел, npm run typecheck:frontend прошел. Manual /users runtime parity не выполнена локально, потому что frontend server не отвечал на 127.0.0.1:5173 и localhost:5173.`
