...
- `e2e/` — сквозные проверки `FEATURE-*`, которые поддерживает `QA`; штатный маршрут запуска проходит вне `ci.yml` и `deploy.yml` против уже развернутого `test`-окружения VPS.
...
- `package.json` — корневой `npm` workspace и команды запуска/сборки/тестирования для серверной части, `apps/backoffice-web`, Playwright e2e-набора вне штатного маршрута непрерывной интеграции и доставки, а также generic-команда проверки контейнерного маршрута `npm run deploy:config` и команда дымовой проверки.
