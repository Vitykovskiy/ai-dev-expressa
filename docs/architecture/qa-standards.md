...
- Для любой `FEATURE-*`, меняющей пользовательский интерфейс, QA обязан проверить идентичность реализации с UI-контрактом из `docs/system/ui-contracts/*` и соответствующим `.references` источником.
