...
- `.github/workflows/` — GitHub Actions workflow `ci.yml` и `deploy.yml`: `ci.yml` выполняет только non-e2e проверку запроса на слияние и формирует обязательные статусы GitHub `quality` и `deployment-validation`, `deploy.yml` автоматически разворачивает `main` в постоянное `test`-окружение на VPS и оставляет `production` только для ручного выпуска через `workflow_dispatch`.
- `docs/` — проектная документация.
- `infra/` — операционные артефакты: generic `docker compose`, шаблоны окружения, generic-скрипт развёртывания и feature-специфичная дымовая проверка `FEATURE-001`.
- `e2e/` — сквозные проверки `FEATURE-*`, которые поддерживает `QA`; штатный маршрут запуска проходит вне `ci.yml` и `deploy.yml` против уже развернутого `test`-окружения VPS, а целевой внешний запуск использует `E2E_BASE_URL=<BACKOFFICE_PUBLIC_URL> npm run test:e2e`.
...
- `package.json` — корневой `npm` workspace и команды запуска/сборки/тестирования для серверной части, `apps/backoffice-web`, Playwright e2e-набора вне штатного маршрута непрерывной интеграции и доставки как локально, так и против развернутого `test`-окружения VPS, а также generic-команда проверки контейнерного маршрута `npm run deploy:config` и команда дымовой проверки.
...
- Любая карточка из `tasks/` перед реализацией проходит обязательную подготовку контекста: агент сначала формирует общий план `<TASK-ID>-execution-plan.md` и контекстные пакеты `<TASK-ID>-context-<NN>-<slug>.md` в корне проекта.
- Подготовка контекста выполняется через субагентов-сборщиков контекста, если среда поддерживает субагентов; основной агент остается координатором и не выполняет исходную задачу на этом этапе.
- Контекстный пакет подзадачи оформляется по `templates/context-package-template.md` и должен содержать цель, поведенческий промпт исполнителя, read set, ключевые факты, зоны правок, зависимости, проверки, критерии готовности и риски.
...
- `DO-*` отвечает только за окружение на VPS, GitHub Actions, маршруты развёртывания и дымовую проверку; e2e не входят в зону ответственности `DO-*`.
- `QA-*` обязательно создаёт и поддерживает e2e-тесты для каждой фичи, запускает их на развернутом `test`-окружении VPS и оформляет воспроизводимые дефекты по найденным проблемам.
...
- Для подготовки контекстных пакетов используйте `templates/context-package-template.md`.
