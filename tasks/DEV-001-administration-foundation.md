...
- Ожидаемый результат для ревью: `Собранная delivery unit DU-01 с дочерними задачами архитектора, frontend, backend, devops и QA дает рабочий административный контур, который можно отдельно развернуть, проверить и показать stakeholder.`
- Проверки: `Собранный результат проходит обязательные проверки дочерних задач FE/BE/DO и финальный приемочный сценарий administrator.`
