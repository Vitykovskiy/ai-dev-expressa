...
- DevOps-owned route для `DO-003` предоставляет `scripts/run-test-vps-e2e.sh` и root wrappers `npm run test:vps:e2e:preflight` / `npm run test:vps:e2e` с режимом preflight и запуском allowlisted QA-owned e2e scenario against deployed `test`.
- GitHub entrypoint для QA — ручной workflow `Test VPS E2E`, который использует `environment: test`, input `scenario`, существующие secrets/vars, SSH на VPS и запускает wrapper в `TEST_VPS_APP_DIR`; он не является `PR Checks` или `Deploy Test` gate.
...
- `TEST_E2E_SCENARIO` задаёт allowlisted сценарий полного запуска; поддерживаемый сценарий v1: `menu-catalog-vps`, который wrapper мапит на `npm run test:e2e:menu-catalog:vps --workspace @expressa/backend`.
- `TEST_E2E_COMMAND` и `TEST_E2E_COMMAND_B64` сохранены только для локальной отладки/backward compatibility wrapper; GitHub workflow не принимает произвольную shell-команду.
- Опциональные входные параметры route: `TEST_E2E_ENV_FILE`, `ENV_FILE`, `TEST_E2E_COMMAND`, `TEST_E2E_COMMAND_B64`, `TEST_E2E_ARTIFACT_DIR`, `TEST_E2E_HEALTH_PATH`, `TEST_E2E_API_PROBE_PATH`, `TEST_E2E_FRONTEND_PATH`, `TEST_E2E_CURL_TIMEOUT`, `TEST_E2E_STAND_COMMIT`, `TEST_E2E_REMOTE_SSH_TARGET`, `TEST_E2E_REMOTE_SSH_PORT`, `TEST_E2E_REMOTE_APP_DIR`.
...
- `TEST_E2E_SCENARIO` задаёт allowlisted QA-owned e2e scenario; v1 поддерживает `menu-catalog-vps`.
- `TEST_E2E_COMMAND` задаёт QA-owned e2e command для локальной отладки wrapper; не требуется в `--preflight-only`, если задан `TEST_E2E_SCENARIO`.
- `TEST_E2E_COMMAND_B64` задаёт base64-encoded QA-owned e2e command для локальной отладки/backward compatibility.
